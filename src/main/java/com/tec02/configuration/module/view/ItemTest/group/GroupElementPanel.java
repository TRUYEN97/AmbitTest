 ...
 */
public class GroupElementPanel extends AbsElementTab<ItemTestDto> {
    ...
    public GroupElementPanel(String tabName, ItemTestDto model) {
        ...
        selectmenu.addItemMenu("Copy", (e) -> {
            ...
            if (obSelected instanceof ItemConfig itemConfig) {
                ...
                groupName = groupName.replaceAll(GROUP_NAME_REGEX, "");
                String newName = JOptionPane.showInputDialog("New name: " + groupName, groupName + "_1");
                if (newName == null || newName.isBlank()) {
                    return;
                }
                ItemGroupDto newGroupDto = MyObjectMapper.convertValue(
                        model.getGroups().get(groupName), ItemGroupDto.class);
                this.model.getGroups().put(newName, newGroupDto);
            }
            this.tabCurrentPanel.update();
            this.tabParentPanel.refesh();
        });
        this.modeListTabel.setDoubleClickAction((input) -> {
            showItemSelected(this.modeListTabel);
        });
        this.applyListTabel.setDoubleClickAction((input) -> {
            showItemSelected(this.applyListTabel);
        });
    }
    private static final String GROUP_NAME_REGEX = "\\*G\\-";

    public ItemGroupDto getThisModel() {
        return thisModel;
    }

    private void showItemSelected(MyListTabel listTabel) {
        Map<String, ItemConfig> items = this.model.getItems();
        Object objectSelected = listTabel.getSelectioned();
        if (objectSelected instanceof ItemConfig itemConfig) {
            ...
            if (!name.startsWith("*G-") && items.containsKey(name)) {
                showConfig.display(items.get(name));
            } else {
                tabCurrentPanel.setSelectedTab(name.replaceAll(GROUP_NAME_REGEX, ""));
            }
        }
    }
    ...
    private synchronized void refeshListModel() {
        ...
        for (String item : itemsInConfig) {
            if (items.containsKey(item)
                    || gourpNames.containsKey(item.replaceAll(GROUP_NAME_REGEX, ""))) {
                this.applyListTabel.addItem(item);
            }
        }
        for (String groupName : gourpNames.keySet()) {
            this.modeListTabel.addItem(String.format("*G-%s", groupName));
        }
